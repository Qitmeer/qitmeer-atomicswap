...
class Contract {
    ...
    _methods(methods, ...arg) {
        ...
        const m = {
            ...
            send: async (address, privateKey) => {
                if (privateKey === undefined) throw 'need privateKey';
                const serialize = await this.sign({ _to: this.contractAddress, _from: address, _value, _data }, privateKey);
    ...
    async estimateGasPriceWeb3({ speed = 'fast' } = {}) {
        ...
        const gasPrice = await new Promise((resolve, reject) => this.web3.eth.getGasPrice((err, gasPrice) => {
            if (err) {
                reject(err);
            } else {
                resolve(gasPrice);
    ...
    async sign({ _to, _from, _value, _data, speed }, privateKey) {
        const web3 = this.web3;
        const t = { to: _to, from: _from, value: _value, data: _data };
        const [nonce, gaspricr] = await Promise.all([web3.eth.getTransactionCount(_from), this.estimateGasPriceWeb3({ speed })]);
        t.gasPrice = web3.utils.toHex(gaspricr);
        t.nonce = web3.utils.toHex(nonce);
        t.gasLimit = web3.utils.toHex('300000');
