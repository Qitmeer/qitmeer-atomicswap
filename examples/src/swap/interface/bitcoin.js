...
class Bitcoin {
  ...
  calculateTxSize({ txIn, txOut = 2 } = {}) {
    const txSize = txIn > 0 ? txIn * 146 + txOut * 33 + (15 + txIn - txOut) : 226; // default tx size for 1 txIn and 2 txOut
    return txSize;
  }

  getFeersBySatoshis({ txIn = 1, txOut = 2, speed }) {
    return this.estimateFeeRate(speed).then(fees => this.calculateTxSize({ txIn, txOut }) * fees);
  }

  fetchUnspentsByTxs(address, addrAmount, page = 0) {
    return request.get(`${this.root}/txs?address=${address}&pageNum=` + page, {}).then(async ({ pagesTotal, txs }) => {
  ...
  fetchUnspents(address, addrAmount, { speed, txOut = 2 } = {}) {
    // 10 seconds cache
    // query requests
    return this.getBalance(address).then(async ({ utxo }) => {
      ...
      for (let i = 0; i < len; i++) {
        ...
        feer ? feers = BigNumber(feer).multipliedBy(1e-8).multipliedBy(this.calculateTxSize({ txIn: i + 1, txOut })).div(1024) : '';
  ...
  estimateFeeRateEARNCOM({ speed } = {}) {
    ...
    // 10 minuts cache
    // query request
    return request.get(`${EARN_COM}`, {}).then(fees => Number(fees[_speed]) * 1024).catch(error => console.log(error));
  }

  fetchTx(hash) {
    // 5 seconds cache
    // query request
    return request.get(`${this.rootcypher}/txs/${hash}`, {}).then(tx => {
  ...
  fetchOmniBalance(address, assetId = 31) {
    return request.urlencoded(`${OMIN}/v1/address/addr/`, {
      ...
      const findById = balance.filter(asset => parseInt(asset.id) === assetId || asset.id === assetId);
      if (!findById.length) {
        return 0;
      }
