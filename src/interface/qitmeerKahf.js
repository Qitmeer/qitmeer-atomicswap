...
class Meer {
  ...
  faucet ( addr, amount ) {
      return request.get(`${this.root}/token/faucet`, {addr, amount})
        .then( e => {
  ...
  meerSearch ( condition ) {
    return request.get(`${this.root}/record`, {condition})
      .then( e => {
          if ( e.code !== 0 ) return [ null, e ]
          return [ e.rs, null ]
      })
  ...
  balance( addr ) {
      return request.get(`${this.root}/stats`, {addr})
        .then( e => {
            if ( e.code !== 0 ) return [ null, e ]
            return [ e.rs, null ]
        })
  ...
  fetchUnspents(addr) {
      // 10 seconds cache
      // query requests
      return request.get(`${this.root}/utxo`, {addr})
        .then( e => {
            if ( e.code !== 0 ) return [ null, e ]
